    ...
    else if (!strcmp(variable, "xcoord"))
  {
    ...
    else if(!strcmp(variable, "servo")) // 3250, 4875, 6500
    {
      ...
      if (val==1) {
        ...
        int ts = speed/2;
        if      (ts < 100) ts = 100;
        if      (actstate == fwd) { ledcWrite(4,ts); ledcWrite(6,    0); }
        else if (actstate == rev) { ledcWrite(4,    0); ledcWrite(6,ts); }
        else                      { ledcWrite(4,ts); ledcWrite(6,ts); }
        delay(100);   
        ...
        ledcWrite(6,0); 
          int ts = speed/2;
        if      (ts < 100) ts = 100;
        if      (actstate == fwd) { ledcWrite(3,    0); ledcWrite(5,ts); }
        else if (actstate == rev) { ledcWrite(3,ts); ledcWrite(5,    0); }
        else                      { ledcWrite(3,ts); ledcWrite(5,ts); }
        delay(100);              
      }
      else if (val==5) {
